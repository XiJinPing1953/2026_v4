...
启动：`npm run preview:statement`。客户对账打开 http://127.0.0.1:5190/#/pages/customer/statement ；出纳登记打开 http://127.0.0.1:5190/#/pages/cashier/receipt-intake 。服务只绑定本机。对账页可切换 kg/瓶/m3；出纳页提供 72 条模拟登记、每页 20 条游标分页，以及“一次上传失败”“一次保存回执超时”开关。重置按钮或刷新恢复初始模拟数据。
...
边界：预览插件仅在显式环境开关下替换 callCloud、auth、App.vue、statement 路由壳和 cashier 路由壳；主业务组件与服务代码均未替换。CSP 将连接限制为本站及本地开发 WebSocket。auth 直接返回本地示例用户，不读取 token 或真实客户；出纳路由壳同时把凭证上传和临时 URL 解析替换为本地 `cloud://demo-space/` 标识。所有 mock 写入仅保存到当前浏览器内存。示例汇总为固定全量范围，日期选择用于界面和分配目标交互，不作为期间会计口径验证。
