...
### 2026-08-24 CURRENT — 储罐网关第二屏与声音辅助报警
- 做了什么：
  - 为 Windows 储罐网关增加第二屏监控窗口：检测到外接显示器后自动全屏显示，主界面可手动打开，未检测到第二屏时提供预览窗口。
  - 增加液位、压力可配置超限报警、通信异常报警、连续超限延时、声音测试、监控屏静音/解除静音和报警状态展示。
  - 声音通过网关电脑的 Windows 音频输出播放，不增加继电器板；阈值留空时不启用对应的液位/压力报警，通信报警默认启用。
- 改动文件列表：
  - `apps/tank-gateway/alarm.cjs`
  - `apps/tank-gateway/main.cjs`
  - `apps/tank-gateway/preload.cjs`
  - `apps/tank-gateway/renderer/index.html`
  - `apps/tank-gateway/renderer/renderer.js`
  - `apps/tank-gateway/renderer/display.html`
  - `apps/tank-gateway/renderer/display.js`
  - `apps/tank-gateway/renderer/display.css`
  - `apps/tank-gateway/README.md`
  - `scripts/tankGatewayAlarm.test.cjs`
  - `package.json`
  - `STATE.md`
- 验证输出要点：
  - 所有新增/修改 Electron、报警和渲染 JS 执行 `node --check`（通过）。
  - `node --test scripts/tankGatewayAlarm.test.cjs scripts/tankTelemetryCore.test.cjs`（通过，10/10）。
  - `git diff --check`（通过）。
  - 未在实际 Windows 双屏设备、真实音响和现场 PLC 上做实机验证。
- 剩余问题：
  - 现场使用前需要填写经设备/工艺人员确认的液位和压力报警阈值，并点击“测试声音”。
  - 该声音报警仅作为显示终端和辅助提醒，不能替代现场安全联锁；Windows关机、断电或音频设备故障时不会发声。

### 2026-08-25 CURRENT — 储罐液位/压力四级声音报警
- 做了什么：
  - 液位报警改为直接比较 PLC 上报的 `level_kpa`，配置低低限、低限、高限、高高限四个独立阈值；液位百分比仅保留为画面填充显示。
  - 压力报警增加低低限、低限、高限、高高限四个独立 MPa 阈值；同一指标同时命中时只保留更严重的一级，避免重复播报。
  - 监控屏声音改为持续音源，报警恢复、网关停止或手动静音时停止；保留单独的测试声音。
- 改动文件列表：
  - `apps/tank-gateway/alarm.cjs`
  - `apps/tank-gateway/main.cjs`
  - `apps/tank-gateway/renderer/index.html`
  ...
  - `scripts/tankGatewayAlarm.test.cjs`
- 验证输出要点：
  - Electron、报警和渲染 JS `node --check`（通过）。
  - `node --test scripts/tankGatewayAlarm.test.cjs scripts/tankTelemetryCore.test.cjs`（通过，11/11）。
  - `git diff --check`（通过）。
- 剩余问题：
  - 阈值需要按现场工艺确认值填写；Windows 双屏、音响和 PLC 实机仍需现场验收。

### 2026-08-25 UPDATE — 四级报警网关安装包
- 验证输出要点：
  - `npm run tank:gateway:dist:win`（通过，生成 Windows x64 NSIS 安装包）。
  - `app.asar` 内容检查确认包含 `alarm.cjs`、`main.cjs`、`display.html`、`display.js` 和 `display.css`。
  - 安装包 SHA-256：`4fd00d1ee3444275ba6669916fa6453d966edcbd4d9efaddc8dfb4d35088557a`。
- 安装包路径：`release/tank-gateway/新拓储罐网关 Setup 1.0.1.exe`。
- 剩余问题：
  - 仅完成 macOS 交叉打包和内容校验，尚未在目标 Windows 电脑用实际双屏、音响和 PLC 做现场验收。

### 2026-08-25 UPDATE — 仪表参数表式报警配置
- 做了什么：
  - 主界面将报警设置改为液位、压力两套独立参数表，分别展示当前值、测量范围上下限、修正值、报警高高/高/低/低低限和报警功能开关。
  - 修正值只参与网关本地报警比较和参数表当前值显示，不修改 PLC 原始读数和云端上报；压力表的示例提示值对应现场截图的 `1.00/0.90/0.10/0.05 MPa`。
  - 配置版本升至 5，并兼容旧版总报警开关配置。
- 改动文件列表：
  - `apps/tank-gateway/alarm.cjs`
  - `apps/tank-gateway/main.cjs`
  - `apps/tank-gateway/renderer/index.html`
  - `apps/tank-gateway/renderer/renderer.js`
  - `apps/tank-gateway/renderer/styles.css`
  - `apps/tank-gateway/README.md`
  - `scripts/tankGatewayAlarm.test.cjs`
- 验证输出要点：
  - 网关 JS `node --check`（通过）。
  - `npm run tank:gateway:test`（通过，13/13）。
  - 参数表字段检查、`git diff --check`（通过）。
  - `app.asar` 解包检查确认参数表和修正值逻辑已打入安装包。
  - 新安装包 SHA-256：`01d18edf0566f79e74614f5a9bac607d52cfb1a28602c0684204bf72e72e6e38`。
- 安装包路径：`release/tank-gateway/新拓储罐网关 Setup 1.0.1.exe`。
- 剩余问题：
  - 参数表仍需按现场确认值填写；尚未在目标 Windows 设备连接真实双屏、音响和 PLC 做现场验收。

### 2026-08-25 UPDATE — 最终参数表安装包
- 在最终打包前补充了未保存表单直接探测/登录/启动时的修正值同步，避免参数表当前值显示旧配置。
- 最终安装包已重新生成并完成 `app.asar` 内容检查。
- 最终安装包路径：`release/tank-gateway/新拓储罐网关 Setup 1.0.1.exe`。
- 最终 SHA-256：`a1f4ebf8382bac42abf6c7b5063ccf5c7b2a703edfefcad69017c67747797a01`。

### 2026-08-25 UPDATE — 参数表两列错位修复
- 将参数表改为严格的两列行布局：标题固定在左侧，数值与单位合并在右侧同一单元格；液位、压力均已同步修复。
- `app.asar` 内容检查确认新 HTML/CSS 已进入安装包。
- 最终安装包路径：`release/tank-gateway/新拓储罐网关 Setup 1.0.1.exe`。
- 最新 SHA-256：`e8f20a6fbb016bff5ec621c99856f04bbcf2b2dcfcdc8d7a1790e4c3865d28ed`。

### 2026-08-25 UPDATE — 报警确认与自动复位
- 增加监控屏“确认报警”按钮：确认后保留报警横幅和当前值，停止未确认报警声音；不允许手动隐藏仍处于超限状态的报警。
- 液位/压力恢复正常后自动清理确认状态，下一次新的报警重新发声。
- 验证：JS 语法检查、`npm run tank:gateway:test`（13/13）、`git diff --check` 和 `app.asar` 内容检查均通过。
- 最终安装包路径：`release/tank-gateway/新拓储罐网关 Setup 1.0.1.exe`。
- 最新 SHA-256：`02d5a2c77a6f6023fef23be1390b369112e738416ec8eca98eb54afc291424d9`。

### 2026-08-26 CURRENT — CRM 根入口与发布前置检查修复
- 做了什么：
  - 恢复项目根 `index.html` 为 uni-app CRM 标准入口；储罐网关继续使用 `apps/tank-gateway/renderer/index.html`，不再与 CRM 共用根入口。
  - 在 `scripts/releaseWeb.cjs` 增加发布前根入口校验，检测到网关页面混入时立即停止，避免再次生成错误的 CRM 发布包。
- 改动文件列表：
  - `index.html`
  - `scripts/releaseWeb.cjs`
  - `STATE.md`
- 验证输出要点：
  - `npm run build:h5`（通过，产物包含 CRM 页面及 `/pages/bottle/inspection`）。
  - `npm run build:mp-alipay`（通过）。
  - `node --check scripts/releaseWeb.cjs`、根入口校验和 `git diff --check`（通过）。
  - 本轮未上传云函数、未重新发布网页。
- 剩余问题及 Next：
  - 线上静态托管仍是此前的旧发布包；后续需在不向公开前端注入云空间密钥的前提下，重新生成带正确 uniCloud 绑定的 CRM 包后再发布。
