...
- **云端重复执行补验未完成**：正式提交后第一次重试返回旧只读入口的400信息，随后状态查询因旧登录令牌失效返回403。旧响应原因尚未确定；不能把本地幂等通过写成生产重复执行已通过。正式提交回执、提交后原始完整回读及实际页面已确认本批结果。后续使用工具前先核验运行规则与既有批次状态，查明旧入口响应；不重新执行K006修账。
