...
const CUSTOMER_ID = '694045c0adf6dbd796e26216'
...
async function snapshot(db) {
 ...
 if (!customer || customer.name !== '陈建磊板厂') throw Error('客户范围不符')
...
exports.main = async (event = {}) => {
  ...
  const runId = batchId(), logId = `k005_reconcile_${runId}`
  try {
    ...
    if (!user || user.role !== 'superadmin') return { code: 403, msg: '仅超级管理员可执行K005核准批次' }
    ...
    if (event.action === 'prepareV1') {
      ...
      await db.collection('crm_operation_logs').add({ _id: logId, action: 'k005_accountant_reconciliation', request_id: runId,
  ...
  } catch (error) { return { code: 409, error_code: 'K005_RECONCILIATION_REJECTED', msg: error.message,
