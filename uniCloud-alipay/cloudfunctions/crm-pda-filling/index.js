...
async function finishTargetWriteV1(user, data = {}) {
	...
	// A late gateway acknowledgement must not reopen a physically completed task.
	if (task.completion_intent) delete patch.status
	const updated = await tasks.where({ _id: task._id, completion_intent: task.completion_intent ? dbCmd.exists(true) : dbCmd.exists(false) }).update(patch)
