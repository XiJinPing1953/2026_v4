...
test('K016受保护批次：失败回滚、冲突、幂等及三账务入口',async()=>{
 const before=JSON.parse(fs.readFileSync(process.env.K016_SOURCE)).data,t=structuredClone(before.tables),cid=before.customer_id
 ...
 const req={customer_id:cid,operation_id:'k016-reconciliation-20260916-v1',expected_snapshot_hash:before.snapshot_hash,evidence:{confirmed_by_user:true,approval_sha256:'a'.repeat(64),source_commit:'b'.repeat(40)}}
 ...
 assert.equal(t.crm_customer_receipts.find(r=>r._id==='6a9fb0226c031c11faccf65c').unallocated_amount,500)
 assert.equal(t.crm_customer_receipts.find(r=>r._id==='6a8e9ed7fbc4c8ef097665ad').unallocated_amount,80)
})
