...
const raw=JSON.parse(fs.readFileSync('outputs/trust-audit/2026-09-13/k005/raw-before.json'))
...
test('K005 real snapshot protected handler rollback, conflict and idempotency',async()=>{
 const t={...structuredClone(raw.tables),crm_users:[{_id:'sa',token:'test',role:'superadmin'}],crm_operation_logs:[]},db=mutableDb(t),handler=loadHandler('crm-k005-reconciliation',db)
