...
test('full rebuild retains a locked truck and scans it after the other worker releases its lease', async () => {
	const db = makeDatabase()
	const handler = loadAnomaly(db)
	const truckNo = 'TRUCK-A12345'
	db.data('crm_users').set('owner', { _id: 'owner', role: 'superadmin', token: 'good' })
	db.data('crm_sale_records').set('s1', { _id: 's1', biz_mode: 'truck', truck_no: truckNo,
		date: '2026-09-05', truck_out_gross: 100, truck_back_gross: null, created_at: 1 })
	const lockId = `scan_${require('node:crypto').createHash('sha256').update(truckNo).digest('hex').slice(0, 40)}`
	const lock = { _id: lockId, lease_id: 'other-worker', lease_until: Date.now() + 120000 }
	db.data('crm_bottle_scan_locks').set(lockId, lock)
	const first = await handler({ action: 'rebuildV2', token: 'good', data: {} }, {})
	assert.equal(first.code, 0)
	assert.equal(first.data.done, false, 'a waiting truck must not become a completed full rebuild')
	assert.equal(first.data.cursor.current_truck_no, truckNo)
	assert.equal(first.data.cursor.truck_after, '')
	assert.equal(first.data.round_trucks, 0)
	assert.equal(db.data('crm_bottle_anomalies').size, 0)
	assert.equal(db.data('crm_bottle_scan_locks').get(lockId).lease_id, 'other-worker')
	lock.lease_until = 0
	const second = await handler({ action: 'rebuildV2', token: 'good', data: { cursor: first.data.cursor } }, {})
	assert.equal(second.code, 0)
	assert.equal(second.data.done, true)
	assert.equal(second.data.cursor, null)
	assert.equal(second.data.round_trucks, 1)
	assert.equal(second.data.round_created, 1, 'the resumed rebuild must inspect the previously skipped truck')
	assert.equal(db.data('crm_bottle_anomalies').size, 1)
})

test('truck scan does not finish until capped anomaly creation and stale resolution have resumed', async () => {
	const db = makeDatabase()
	const handler = loadAnomaly(db)
	const truckNo = 'TRUCK-B12345'
	db.data('crm_users').set('owner', { _id: 'owner', role: 'superadmin', token: 'good' })
	for (let index = 0; index < 121; index++) {
		const _id = `truck-sale-${index}`
		db.data('crm_sale_records').set(_id, { _id, biz_mode: 'truck', truck_no: truckNo,
			date: '2026-09-05', truck_out_gross: 100, truck_back_gross: null, created_at: index + 1 })
	}
	const scan = () => handler({ action: 'scanTruckAnomaliesV1', token: 'good',
		data: { truck_no: truckNo, max_writes_per_round: 120, reconcile_anomalies: true } }, {})
	const first = await scan()
	assert.equal(first.code, 0)
	assert.equal(first.data.round_created, 120)
	assert.equal(first.data.done, false, '121 detected anomalies cannot be complete after only 120 writes')
	assert.ok(first.data.cursor)
	const second = await scan()
	assert.equal(second.data.done, true)
	assert.equal(second.data.round_created, 1)
	assert.equal(db.data('crm_bottle_anomalies').size, 121)
	assert.equal((await scan()).data.round_created, 0, 'retry must not duplicate detected anomalies')
	db.data('crm_sale_records').clear()
	const staleFirst = await scan()
	assert.equal(staleFirst.data.round_resolved_stale, 120)
	assert.equal(staleFirst.data.done, false, 'remaining stale anomalies must keep the scan pending')
	const staleSecond = await scan()
