...
const p=require('../uniCloud-alipay/cloudfunctions/crm-k019-reconciliation/plan'),{snapshot}=require('../uniCloud-alipay/cloudfunctions/crm-k019-reconciliation/snapshot')
test('K019受保护批次：失败回滚、冲突、幂等及三账务入口',async()=>{
 const before=JSON.parse(fs.readFileSync(process.env.K019_SOURCE)).data,t=structuredClone(before.tables),cid=before.customer_id
 t.crm_users=[{_id:'testactor',username:'test',token:'test',role:'superadmin'}];t.crm_operation_logs=[]
 const db=mutableDb(t),h=loadHandler('crm-k019-reconciliation',db),s=loadHandler('crm-customer-settlement',db)
 const req={customer_id:cid,operation_id:'k019-reconciliation-20260916-v1',expected_snapshot_hash:before.snapshot_hash,evidence:{confirmed_by_user:true,approval_sha256:'a'.repeat(64),source_commit:'b'.repeat(40)}}
 ...
 for(const action of ['getCustomerStatementV1','exportCustomerStatementV1','exportCustomerAccountingLedgerV1']){r=await invoke(s,action,{customer_id:cid,date_from:'2026-01-01',date_to:'2026-09-16'});assert.equal(r.code,0,r.msg);const x=r.data.period_summary;if(!x.complete)console.log(JSON.stringify(x));for(const [k,v]of Object.entries({business_revenue:36927,cash_received:36886,refund_total:0,net_cash_received:36886,rounding_total:41,settlement_fee_total:0,complete:true}))assert.equal(x[k],v,action+':'+k);if(action!=='getCustomerStatementV1')assert.equal(r.data.closing_balance,0);if(r.data.summary){assert.equal(r.data.summary.receivable_balance,0);assert.equal(r.data.summary.prepay_balance,0)}}

 assert.deepEqual(t.crm_customers,before.tables.crm_customers)
 for(const r of before.tables.crm_customer_receipts.filter(r=>r.entry_kind==='offset_credit'))assert.deepEqual(t.crm_customer_receipts.find(x=>x._id===r._id),r)
 for(const [date,cash,round]of [['2026-02-07',2289,0],['2026-02-16',730,5],['2026-03-06',1225,0],['2026-04-17',1750,0],['2026-04-24',1449,7],['2026-05-18',820,0],['2026-05-23',905,7.5],['2026-06-03',500,0],['2026-06-26',982,0.5],['2026-07-02',1380,0],['2026-07-13',1717,0.5]]){
  const result=await invoke(s,'getCustomerStatementV1',{customer_id:cid,date_from:date,date_to:date});assert.equal(result.data.period_summary.cash_received,cash,date);assert.equal(result.data.period_summary.rounding_total,round,date)
 }
