...
生产已部署并验收的是上述两个限定客户修正、通用期间汇总及期初预付款识别；09-05 系统可信度全量候选仍未整体部署。旧清单 38 张不能继续写成当前待核数，也不能因两个客户已处理就推算全仓为零。云端全量事务/调度、PDA、Windows 网关和现场硬件仍未验收；销售持久写入链路、全量财务指标拆分、催收流量覆盖和按瓶/客户汇总仍未完成。
