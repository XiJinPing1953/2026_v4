...
import { customerPeriodSummaryRows, normalizeCustomerPeriodSummary, outstandingPeriodIssues, periodReviewLabel, describePeriodSummaryIssue } from '@/services/mappers/customerPeriodSummary.js'
...
export function buildStatementSheetRows(payload = {}) {
	...
	const cashStatus = periodReviewLabel(summary) || ((summary?.cash_complete ?? summary?.complete) === true ? '系统收退款无待核项' : `现金完整性待核${summary ? `（${outstandingPeriodIssues(summary).length}项）` : ''}`)
...
function buildPeriodSummarySheetRows(payload = {}) {
	...
	return [
		...
		...customerPeriodSummaryRows(summary).map(row => [
			...
			row.value == null ? { type: 'String', value: periodReviewLabel(summary) || (summary ? '待核' : '未完成') } : moneyCellByScale(row.value, scale)
		]),
		...
		[{ type: 'String', value: '核查状态' }, { type: 'String', value: periodReviewLabel(summary) || (summary?.complete ? '完整' : summary ? `待核 ${outstandingPeriodIssues(summary).length} 项` : '未完成') }],
		...(summary?.source_notes || []).map(row => [{ type: 'String', value: '期间依据' }, { type: 'String', value: row.text }]),
		...(summary?.manual_review ? [[{type:'String',value:'人工核准'}, {type:'String',value:summary.manual_review.note}]] : []),
		...outstandingPeriodIssues(summary).map(row => [{ type: 'String', value: row.source_id }, { type: 'String', value: describePeriodSummaryIssue(row) }])
	]
}
