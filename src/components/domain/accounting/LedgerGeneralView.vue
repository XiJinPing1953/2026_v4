<template>
	<AppPage title="总账" :subtitle="subtitle" icon="document">
...
const { loading, run: fetchList } = useQuery(
	async () => {
		const res = await getGeneralLedgerV1({
	...
	},
	{
		...
		cacheTTL: 8000,
		throttleMs: 300,
		cacheKey: () => `ledger:general:${filters.accountCode}:${filters.dateStart}:${filters.dateEnd}:${pager.page}:${pager.pageSize}`
	}
)
