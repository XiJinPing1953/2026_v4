<template>
	<AppPage title="明细账" :subtitle="subtitle" icon="list">
		...
		<view class="list-shell">
			<AppSection title="查询条件">
				...
				<view class="filter-grid">
					<AppInput v-model="filters.accountCode" label="科目编码" placeholder="必填" size="sm" />
			...
			<AppSection title="明细">
				...
				<AppList :loading="loading" :empty="list.length === 0" empty-title="暂无明细">
					<AppListItem
						v-for="item in list"
						:key="item._id"
						:title="item.date + ' ' + item.voucher_no"
...
import { getSubLedgerV1 } from '@/services/ledger'
...
const subtitle = computed(() => {
	if (!pager.total) return '按科目查看明细账'
...
const { loading, run: fetchList } = useQuery(
	async () => {
		const res = await getSubLedgerV1({
	...
	},
	{
		...
		cacheTTL: 8000,
		throttleMs: 300,
		cacheKey: () => `ledger:sub:${filters.accountCode}:${filters.dateStart}:${filters.dateEnd}:${pager.page}:${pager.pageSize}`
	}
)
...
async function onSearch(resetPage = false) {
	if (!String(filters.accountCode || '').trim()) {
		uni.showToast({ title: '请先填写科目编码', icon: 'none' })
		return
	}
