		...
		<view v-else-if="record" class="detail-stack">
			...
			<view v-for="area in groupedItems" :key="area.code" class="detail-card">
				...
				<view v-for="item in area.items" :key="item.item_code" :class="['item-row', { 'item-row--danger': item.is_abnormal }]">
					...
					<view class="photo-grid"><image v-for="fileId in item.photo_file_ids" :key="fileId" class="photo" :src="mediaMap[fileId] || fileId" mode="aspectFill" @click="preview(item.photo_file_ids, fileId)" /></view>
					<view v-if="item.is_abnormal" class="hazard-summary">
						...
						<image v-if="item.responsible_signature_file_id" class="signature" :src="mediaMap[item.responsible_signature_file_id] || item.responsible_signature_file_id" mode="aspectFit" />
					</view>
				</view>
			</view>
		</view>
...
import { resolveInspectionFileUrls } from '@/services/stationSafetyInspectionMedia'
...
function preview(fileIds, current) { const urls = fileIds.map((id) => mediaMap.value[id] || id); uni.previewImage({ urls, current: mediaMap.value[current] || current }) }
async function load() {
	loading.value = true
	try {
...
.photo-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 10rpx; margin-top: 12rpx; }.photo { width: 100%; aspect-ratio: 1; border-radius: 12rpx; background: #e2e8f0; }.hazard-summary { display: flex; flex-direction: column; gap: 8rpx; margin-top: 14rpx; padding: 14rpx; color: #881337; border-radius: 12rpx; background: #ffe4e6; font-size: 21rpx; line-height: 1.5; }.signature { width: 100%; height: 160rpx; background: #fff; border-radius: 10rpx; }
</style>
