<template>
	<view class="signature">
		...
		/>
		<text v-if="!hasInk" class="signature__hint">请在此处手写签名</text>
		<view class="signature__tools">
			<text :class="['signature__state', hasInk ? 'signature__state--ok' : '']">
				{{ hasInk ? '已完成手写签名' : '签名必填' }}
			</text>
...
const canvasId = `home-safety-signature-${Math.random().toString(36).slice(2, 10)}`
...
function refreshRect() {
	...
	query.select(`#${canvasId}`).boundingClientRect((value) => {
		if (!value) return
		const sizeChanged =
			Math.abs(Number(rect.value.width || 0) - Number(value.width || 0)) > 1 ||
			Math.abs(Number(rect.value.height || 0) - Number(value.height || 0)) > 1
		rect.value = value
		if (sizeChanged && strokes.length) {
			context = null
...
function normalizePoint(point) {
	const width = Math.max(Number(rect.value.width || 0), 1)
	const height = Math.max(Number(rect.value.height || 0), 1)
	return {
		x: Math.min(Math.max(point.x / width, 0), 1),
		y: Math.min(Math.max(point.y / height, 0), 1)
	}
}

function denormalizePoint(point) {
	return {
		x: point.x * Math.max(Number(rect.value.width || 0), 1),
		y: point.y * Math.max(Number(rect.value.height || 0), 1)
	}
}
...
.signature__canvas {
	...
	height: 280rpx;
	background: #fff;
	border: 2rpx dashed #9fb3c8;
	border-radius: 18rpx;
	box-sizing: border-box;
	touch-action: none;
}
:deep(.signature__canvas .uni-canvas-canvas) {
	touch-action: none;
}
.signature__hint {
	position: absolute;
