...
export function buildRoleTemplatePermissions(roleTemplate) {
	...
	return PAGE_REGISTRY.reduce((acc, item) => {
		...
		acc[item.pagePath] = ACTIONS.reduce((entry, action) => {
			entry[action] = action === 'view' ? Boolean(base.view && item.supports[action]) : Boolean(base[action] && item.supports[action])
			return entry
		}, {})
		return acc
	}, {})
